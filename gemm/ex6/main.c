...
struct parsed_args
{
    ...
    int block_size;  /* tamanho dos blocos */
};
...
static struct parsed_args* parse_args(int argc, const char* argv[])
{
    ...
    for (i = 0; i < argc-1; ++i)
    {
        if (!strcmp(argv[i], "--matrix-size") || !strcmp(argv[i], "-n"))
            ret.matrix_size = atoi(argv[i+1]);
        else if (!strcmp(argv[i], "--algorithm") || !strcmp(argv[i], "-a"))
            ret.algorithm = atoi(argv[i+1]);
        else if (!strcmp(argv[i], "--block-size") || !strcmp(argv[i], "-b"))
            ret.block_size = atoi(argv[i+1]);
    }
    return &ret;
}
...
static bool validate_args(const struct parsed_args* args)
{
    if (args->matrix_size == INVALID_SIZE ||\
        args->algorithm >= INVALID_ALG ||\
        args->block_size == INVALID_SIZE)
        return false;
    return true;
}
...
static void print_usage_message()
{
    ...
    "  main <ARGS>\n"
    "onde:\n"
    "  -n, --matrix-size <NUM>     Tamanho da matriz quadrada. (n_linhas = n_colunas)\n"
    "  -a, --algorithm <NUM>       Número da implementação. (0 = dgemm_0, 1 = dgemm_1, 2 = dgemm_2)\n"
    "  -b, --block-size <NUM>      Tamanho dos blocos utilizados no cálculo\n"
    "\n";
...
int main(int argc, const char* argv[])
{
    ...
    if (!matrix_which_dgemm(args->algorithm, n, C, A, B, bs))
    {
